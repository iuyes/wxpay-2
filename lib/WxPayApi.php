...
class WxPayApi
{
    ...
    public function unifiedOrder($inputObj, $timeOut = 6)
    {
        ...
        if (!$inputObj->IsNotify_urlSet()) {
            $inputObj->SetNotify_url($this->config->getNotifyUrl());//异步通知url
        }
    ...
    public function report($inputObj, $timeOut = 1)
    {
        ...
        // FIXME $startTimeStamp = $this->getMillisecond();//请求开始时间
        $response = $this->postXmlCurl($xml, $url, false, $timeOut);
    ...
    {
        //如果不需要上报数据
        if ($this->config->getReportLevenl() == 0) {
            return;
        }
        //如果仅失败上报
        if ($this->config->getReportLevenl() == 1 &&
    ...
    private function postXmlCurl($xml, $url, $useCert = false, $second = 30)
    {
        ...
        //如果有配置代理这里就设置代理
        if ($this->config->getCurlProxyHost() != "0.0.0.0"
            && $this->config->getCurlProxyPort() != 0
        ) {
